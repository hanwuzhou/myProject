    ...
    success: function(pager){
     ...
     /* $(".pager").createPage({
      ...
      for(var i=1;i<=pageCount;i++){
        lis+=(i!=pageNo?`<a href="#" class="tcdNumber">${i}</a>`: `<a href="#" class="current">${i}</a>`);
      }
      var html=
          `<a href="#" class="prevPage">上一页</a>${lis}<a href="#" class="nextPage">下一页</a>`;

      $(".pager").html(html);

      if(pageNo==1){
        $(".pager a:first-child").addClass("disabled");
      }
      if(pageNo==pageCount){
        $(".pager a:last-child").addClass("disabled");
      }
      if(pageNo!=1 && pageNo!=pageCount){
        $(".pager a:first-child").removeClass("disabled");
        $(".pager a:last-child").removeClass("disabled");
      }
    }
  })
}
...
$(".pager").on("click","a",(e)=>{
  ...
  if(pag.className.indexOf("disabled")==-1){
    var curr=document.querySelector(".pager .current");
    if(pag.innerHTML=="上一页"){
      loadProductByPage(parseInt(curr.innerHTML)-1);
    }else if(pag.innerHTML=="下一页"){
      loadProductByPage(parseInt(curr.innerHTML)+1);
    }else{
      loadProductByPage(parseInt(pag.innerHTML));
    }
  }
})
/**附加导航滚动监听**/
...
$('.store_action_right_cart_content').delegate('.cart_reduc', 'click', function () {
  ...
  if (numone >= 1) {
    ...
    var new_this_price = parseInt(numone) * parseInt(this_price.slice(1));
...
$('.store_action_right_cart_content').delegate('.cart_add', 'click', function () {
  ...
  var id = $(this).parent().parent().attr('id');
  var this_price = $('.salc_content').find("#" + id).find('.price').text();
  var new_this_price = parseInt(numtwo) * parseInt(this_price.slice(1));
  $(this).parent().next('span').find('.cart_unit_price').html(new_this_price.toFixed(2));
  update();
})
...
$(".store_action_right_cart .settle").on("click","a",(e)=>{
  e.preventDefault();
  console.log($(e.target));
  console.log($(e.target).parent().parent().parent().find('.store_action_right_cart_content>.addtion input'));

  var l=$(e.target).parent().parent().parent().find('.store_action_right_cart_content>.addtion');
  $.each(l,function(i,k){
    $.ajax({
      type:'post',
      url:'data/cart/add.php',
      async:false,
      data:{lid:k.id,buyCount:k.querySelector('input').value},
